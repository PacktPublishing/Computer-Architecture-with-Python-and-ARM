...
### MOV r4,#6 @ r4 number of elements to add 11 0000 0100 011000 00000100
...
iClass3 = ['MOV','MOVE','ADD','ADC','SUB','CMP', 'NOT',  # class 11 mnemonic two operands
            'AND','OR','EOR''ONES','MRG','FFO','LSL','LSR']
...
def trace(): # Function to print execution data
    ...
    return()
testCode = "TC3_NEW_1.txt" # Source filename on my computer
...
for i in range (0,len(src)): # Deal with equates of the form EQU PQR 25
    if src[i][0] == 'EQU': # If the line is 3 or more tokens and first token is EQU
        sTab[src[i][1]] = getL(src[i][2]) # Put token in symbol table as integer getL deals with number formatsrc = [i for i in src if i.count("EQU") == 0] 
...
for ic in range(0,len(src)): # ASSEMBLY LOOP (ic = instruction counter)
    ...
    if t0 in iClass0: # If in iClass0 it's a single instruction, no operands
    ...
    elif t0 in iClass3: # Two-operand inst. All dataprocessing and movement ops in iClass3
        if (t1 in rName) and (t2[0] == '#'): # Look for register name and literal for mode 4
            ...
            mode = 11
        elif (t1 in rNamInd) and (t2[0:2] == 'M:'): # Look for register indirect name and literal prefixed by M:
            mode = 12
...
while run == True: # MAIN LOOP
    ...
    inst = pMem[pc] # Fetch current instruction. inst is the binary opcode executed in this cycle
    ...
    elif iClass == 1: # Class 1 operation branch # Class 1 branch and instr with literal operand's
