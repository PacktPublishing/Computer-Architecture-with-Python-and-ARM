...
def memProc(src): # Memory processing 5
    ...
    for i in range(len(src)): # and remove directives from source code
        if src[i][0] == '.WORD': # Test for .word directive
            ...
            memPoint = memPoint + 1 # Move the memory pointer on one word locatio
...
def display(): # Print the state after each instruction
    ...
    print('PC =',f1,'Reg =',b,'Z =',z,'N =',n,\
    'C =',c,f2) # Print the data
    return()
...
codes = {"STOP":(0,0),"NOP":(0,1),"RND":(1,4),"BRA":(2,5),"BEQ":(2,6),
         ...
         "ADD":(5,16),"ADDL":(6,17),"SUB":(5,18),"SUBL":(6,19),"AND":(5,20),
         "ANDL":(6,21),"OR":(5,22),"ORL":(6,23), "EOR":(5,24),"EORL":(6,25),
         "CMP":(3,26),"CMPL":(4,27),"LSL":(3,28),"LSR":(3,29),"ROL":(3,30), 
         "ROR": (3,31), "BSR":(2,32),"RTS":(0,33),"PUSH":(1,34),"POP":(1,35),
         "BL":(2,36),"RL":(0,37), "INC":(1,48), "DEC":(1,49), "PRT":(1,3), 
         "BHS": (2,71)}
